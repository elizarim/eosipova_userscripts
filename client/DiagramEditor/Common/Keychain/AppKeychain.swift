...
class AppKeychain {
    ...
    func set(
        _ value: String,
        forKey key: String,
        withAccess access: AppKeychainAccessOptions? = nil
    ) -> Bool {
    ...
    func set(
        ...
        let accessible = access?.value ?? AppKeychainAccessOptions.defaultOption.value

        let prefixedKey = keyWithPrefix(key)

        var query: [String: Any] = [
            AppKeychainConstants.class: kSecClassGenericPassword,
            AppKeychainConstants.attrAccount: prefixedKey,
            AppKeychainConstants.valueData: value,
            AppKeychainConstants.accessible: accessible
        ]
    ...
    func getData(_ key: String, asReference: Bool = false) -> Data? {
        ...
        var query: [String: Any] = [
            AppKeychainConstants.class: kSecClassGenericPassword,
            AppKeychainConstants.attrAccount: prefixedKey,
            AppKeychainConstants.matchLimit: kSecMatchLimitOne
        ]

        if asReference {
            query[AppKeychainConstants.returnReference] = kCFBooleanTrue
        } else {
            query[AppKeychainConstants.returnData] =  kCFBooleanTrue
        }
    ...
    var allKeys: [String] {
        var query: [String: Any] = [
            AppKeychainConstants.class: kSecClassGenericPassword,
            AppKeychainConstants.returnData: true,
            AppKeychainConstants.returnAttributes: true,
            AppKeychainConstants.returnReference: true,
            AppKeychainConstants.matchLimit: AppKeychainConstants.secMatchLimitAll
        ]
        ...
        if lastResultCode == noErr {
            return (result as? [[String: Any]])?.compactMap {
                $0[AppKeychainConstants.attrAccount] as? String } ?? []
        }
    ...
    func addAccessGroupWhenPresent(_ items: [String: Any]) -> [String: Any] {
        ...
        result[AppKeychainConstants.accessGroup] = accessGroup
        return result
    }
}
